     ...
     <div style={{display: "flex", alignItems: "center", background: "rgba(0,0,0,0.5)", padding: 4, borderRadius: 40, maxWidth: 170}}>
      {((data?.ranking || 0) < 1000)?
        ...
        <div style={{fontSize: "0.8em"}}>
          <div style={{ color: "white", marginLeft: 5, display: "flex", alignItems: "center", fontWeight: 700}}>
            {data?.elo} <span style={{marginLeft: 2, color: data?.eloDiff ? "rgb(50, 211, 90)" : "#ef5351",}}>({(data?.eloDiff || 0) > 0 ? "+" : ""}{data?.eloDiff})</span>
          </div>
          <div style={{marginLeft: 10, fontWeight: 500}}>
            <span style={{color: "rgb(50, 211, 90)"}}>{data?.wins}W</span> <span style={{color: "#efefef"}}>-</span> <span style={{color: "#ef5351"}}>{data?.losses}L</span>
          </div>
        </div>
     </div>
  );
